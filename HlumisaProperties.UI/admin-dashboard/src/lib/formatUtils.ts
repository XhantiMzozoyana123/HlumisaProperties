// Helper: format digits with SA thousand separators using spaces
export function formatNumberWithSpaces(raw: string): string {
  const digits = raw.replace(/\D/g, "");
  if (!digits) return "";
  return digits.replace(/\B(?=(\d{3})+(?!\d))/g, " ");
}

// Helper: format a number input while typing, keeping it parseable
export function formatNumberInput(value: string): string {
  // Allow decimal point for money
  const cleaned = value.replace(/[^\d.]/g, "");
  const [whole, decimal] = cleaned.split(".");
  const formattedWhole = formatNumberWithSpaces(whole);
  return decimal !== undefined ? `${formattedWhole}.${decimal}` : formattedWhole;
}

// Parse a formatted number back to plain digits (for storage/state)
